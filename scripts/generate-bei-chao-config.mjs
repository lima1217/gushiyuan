/**
 * Generate Bei-chao volume catalog config from epub structure.
 * Run: node scripts/generate-bei-chao-config.mjs
 * Output: scripts/bei-chao-config.mjs
 */
import fs from "fs";
...
const OUTPUT_PATH = path.join(__dirname, "bei-chao-config.mjs");
 ...
 * @param {string} dynasty
 */
function parseAuthorSection(section, dynasty) {
  const entries = [];
  const h3Matches = [...section.matchAll(/<h3 class="kindle-cn-heading3"[^>]*>([\s\S]*?)<\/h3>/gi)];
  ...
  for (let i = 0; i < h3Matches.length; i++) {
    ...
    const h4Matches = [
      ...authorSection.matchAll(/<h4 class="kindle-cn-heading1">([\s\S]*?)<\/h4>/gi),
    ];

    if (h4Matches.length === 0) {
      const poemBlocks = [
        ...authorSection.matchAll(/<p class="kindle-cn-poem-left">([\s\S]*?)<\/p>/gi),
      ];
    ...
    for (let j = 0; j < h4Matches.length; j++) {
      ...
      const h4End =
        j + 1 < h4Matches.length ? h4Matches[j + 1].index : authorSection.length;
      const h4Section = authorSection.slice(h4Start, h4End);
      const blocks = [
      ...
      entries.push({ author, title, dynasty, mode });
    }
  }
...
/**
 * @param {Array<{ author: string, title: string, dynasty: string, mode: string }>} entries
 */
